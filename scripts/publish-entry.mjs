...
if (!lifecycle || lifecycle.status !== "reviewed") throw new Error("This language version must be reviewed before publication.");
...
const published = { ...lifecycle, status: "published", publishedAt: localDateTime(), publishedFrom: decision === "--as-is" ? "original" : "draft", publishedSha256: sha256(publishedBody), authorDecision: decision === "--as-is" ? "publish_as_is" : "publish_current_draft" };
if (locale === metadata.originalLanguage) await writeMetadata(slug, { ...metadata, ...published });
