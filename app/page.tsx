      ...
      <section className="feed" aria-labelledby="journal-heading">
        ...
        {publishedEntries.length === 0 ? (
          ...
          <article className="feed-entry" key={entry.slug}>
            <time dateTime={entry.publishedAt}>{formatDate(entry.publishedAt, entry.originalLanguage)}</time>
            <h2><Link href={`/entries/${entry.slug}`}>{entry.title}</Link></h2>
            {entry.excerpt && <p className="excerpt">{entry.excerpt}</p>}
            {entry.tags.length > 0 && <ul className="tags" aria-label="Tags">{entry.tags.map((tag) => <li key={tag}>{tag}</li>)}</ul>}
          </article>
        ))}
      </section>
    </main>
